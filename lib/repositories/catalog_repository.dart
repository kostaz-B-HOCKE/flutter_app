...
Future<List<Catalog>> getAllCatalogs() async {
  try {
    ...
    return (response as List<dynamic>)
        .map<Catalog>((catalogData) => Catalog.fromMap(catalogData as Map<String, dynamic>))
        .toList();
  } catch (e) {
...
Future<Catalog?> getCatalogById(int id) async {
  try {
    ...
    return Catalog.fromMap(response as Map<String, dynamic>);
  } catch (e) {
...
Future<void> moveCatalog(int catalogId, int newPosition) async {
  try {
    ...
    final Catalog? catalog = await getCatalogById(catalogId);
    if (catalog == null) return;
    
    final int? currentPosition = catalog.sortOrder;
    if (currentPosition == null) return;
    
    if (newPosition == currentPosition) return;
    
    final List<Catalog> catalogs = await getAllCatalogs();
    
    if (newPosition > currentPosition) {
      // Двигаем вниз - уменьшаем позиции между current+1 и new
      for (final Catalog cat in catalogs) {
